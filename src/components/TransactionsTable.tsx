  ...
  return (
    <div className="app-card">
      ...
      <div className="space-y-4 md:hidden">
        {filtered.map((t) => (
          <div key={t.id} className="rounded-3xl border border-gray-200 bg-slate-50 p-4 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{formatDate(t.date)}</p>
                <p className="mt-2 text-sm font-semibold text-slate-900 truncate">{t.description}</p>
                <span
      ...
      {filtered.length === 0 && (
        <p className="text-center text-slate-500 py-8 text-sm">No transactions found</p>
      )}
    </div>
  )
}