...
export function LandingPage({ onGetStarted }: { onGetStarted: () => void }) {
  ...
  return (
    <div className="bg-[#F7F5F0] overflow-hidden relative">
      ...
      <div className="relative z-10">
        ...
        >
          <div className="flex items-center gap-2">
            <span className="bg-[#2F5D4E] text-white w-9 h-9 rounded-lg flex items-center justify-center text-sm">
              📊
            </span>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-600">
            {navLinks.map(({ label, href }) => (
              <a key={label} href={href} className="relative group" onClick={(e) => handleNavClick(e, href)}>
                {label}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="hidden md:flex bg-[#2F5D4E] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#264A3E] transition-colors items-center gap-2"
          >
            Sign Up →
          </motion.button>
          <button
        ...
        {/* Hero */}
        <main className="max-w-7xl mx-auto px-6 sm:px-12 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center py-10">
          <motion.div
              ...
              {[
                {
                  ...
                  desc: "Add and organize with ease",
                },
                {
                  icon: "📊",
                  title: "Visualize Spending",
                  desc: "Clear charts & insights",
                },
                {
                  icon: "🛡️",
          ...
          >
            <motion.img
              ...
              className="w-full max-w-full md:max-w-225 object-cover rounded-lg shadow-[#2F5D4E]/20 shadow-xl"
